...
export function ProjectError({ message, retry }: { message: string; retry?: () => void }) {
  return <div role="alert" className="rounded-2xl border border-red-200 bg-red-50 p-5 text-red-900"><p className="font-semibold">We couldn&apos;t load this project.</p><p className="mt-1 text-sm">{message}</p>{retry && <button type="button" onClick={retry} className="mt-4 rounded-xl bg-red-700 px-4 py-2 text-sm font-semibold text-white hover:bg-red-800">Try again</button>}</div>;
}

export function BackLink({ href = "/projects" }: { href?: string }) {
  return <Link href={href} className="text-sm font-semibold text-emerald-700 hover:text-emerald-900">← Back to projects</Link>;
}
