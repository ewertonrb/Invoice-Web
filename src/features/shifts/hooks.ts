...
import { acceptShift, cancelShift, cancelShiftAssignment, createShift, declineShift, getAvailableShifts, getMyShifts, getShifts } from "./api";
...
export function useCancelShiftAssignment(companyId: number, shiftId: number) { const client = useQueryClient(); return useMutation({ mutationFn: (assignmentId: number) => cancelShiftAssignment(companyId, shiftId, assignmentId), onSuccess: () => client.invalidateQueries({ queryKey: queryKeys.shifts(companyId) }) }); }
